...
// A bare decade token: "80s", "1980s", "80", "1980", "80's"/"1980's"
// (apostrophe-s), "80er"/"1980er" (German). Used as a whole-word match
// against multi-word tags too, so "90s y más" or "80s rock" are still
// recognized as decade-related even though the full tag isn't just a
// decade on its own.
const DECADE_TOKEN = /^(19|20)?\d{2}('s|s|er)?$/i;

// Station frequencies/call-signs ("100.1 fm", "90.3", "88.1 fm") and
// bitrate/format specs ("64kbps", "128 kbit/s", "320 kbit/s") — real tag
// data, but describe the stream, not the station, so they're dropped
// outright rather than just deprioritized.
const FREQUENCY_TAG_PATTERN = /^\d{1,4}([.,]\d{1,2})?\s*(fm|am|khz|mhz)?$/i;
const BITRATE_TAG_PATTERN = /^\d{2,4}\s*[- ]?\s*k(b(it)?)?(ps|\/s)?$/i;
const JUNK_TAGS = new Set(['aac', 'aac+', 'mp3', 'ogg', 'wav', 'flac', 'ssl', 'http', 'https', '24/7', '247']);

// Whole-word helper: DISCOVER_GENRES entries can be multi-word ("hip hop",
// "drum and bass"), and tags can carry a genre as part of a longer phrase
// ("acid jazz", "80s rock", "70s soft rock") — a plain Set lookup only
// catches an exact match, so this checks for the genre as a word-bounded
// substring instead.
function containsGenre(name: string, genres: string[]): string | null {
  const lower = name.toLowerCase();
  for (const genre of genres) {
    const pattern = new RegExp(`(^|[^a-z0-9])${genre.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}([^a-z0-9]|$)`, 'i');
...
function isNoiseTag(name: string): boolean {
  if (JUNK_TAGS.has(name.toLowerCase())) return true;
  if (FREQUENCY_TAG_PATTERN.test(name) || BITRATE_TAG_PATTERN.test(name)) return true;
  // No letters or digits at all ("#", "-", "...") — not a real tag.
  if (!/[a-z0-9]/i.test(name)) return true;
  return false;
}

function isDecadeAdjacent(name: string): boolean {
  return name.split(/\s+/).some(word => DECADE_TOKEN.test(word));
}

// rebalanceTopTags fixes the raw catalog's tag-frequency data being
// dominated by noise — decade tags and decade-flavored phrases (70s,
// 1970, 80s rock, 90s y más, ...), station frequencies, and stream-format
// tags (bitrate, codec) all crowd out genuinely interesting content tags
// in a plain frequency sort. Frequencies and format noise are dropped
// outright since they're not a meaningful way to browse; decade-flavored
// tags stay since browsing "80s" radio is legitimate, just capped — unless
// the tag also names a real genre ("80s rock", "acid jazz"), in which case
// the genre signal wins and it's promoted to the front instead of capped.
export function rebalanceTopTags(tags: TopTag[], maxDecadeTags: number = 6, total: number = 30): TopTag[] {
  const genreMatches: TopTag[] = [];
  ...
  for (const tag of tags) {
    ...
    if (containsGenre(name, DISCOVER_GENRES)) {
      genreMatches.push(tag);
    } else if (isDecadeAdjacent(name)) {
      decadeTags.push(tag);
    } else {
      otherTags.push(tag);
    }
  ...
  return [...genreMatches, ...otherTags, ...decadeTags.slice(0, maxDecadeTags)].slice(0, total);
}
