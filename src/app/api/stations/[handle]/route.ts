...
export async function PATCH(request: NextRequest, { params }: { params: { handle: string } }) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return Response.json(
        { error: errorData.message || 'Failed to update station' },
    ...
    return Response.json({ error: 'Failed to update station' }, { status: 500 });
  }
}
