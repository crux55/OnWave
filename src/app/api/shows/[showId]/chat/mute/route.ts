...
export async function POST(request: NextRequest, { params }: { params: { showId: string } }) {
  ...
  try {
    ...
    const response = await fetch(`${apiHost}/shows/${params.showId}/chat/mute`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      ...
      return Response.json(
        { error: errorData.message || 'Failed to mute user' },
    ...
    return Response.json({ error: 'Failed to mute user' }, { status: 500 });
  }
}
