...
import { Loader2, Radio, CalendarClock, CircleOff, Play, Pause, Users, X, Share2 } from 'lucide-react';
...
export default function ShowDetailClient() {
  ...
  // A native share sheet on mobile beats a plain copy — but browsers
  // without navigator.share (most desktop ones) and any share() the user
  // cancels or that outright fails still need a clipboard link, so this
  // covers all three outcomes with one toast at the end of each path.
  const handleShare = useCallback(async () => {
    if (!show) return;
    const url = `${window.location.origin}/shows/${show.id}`;
    const shareData = { title: show.name, text: `Listen to ${show.name} on OnWave`, url };
    if (navigator.share) {
      try {
